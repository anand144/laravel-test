				...
				<section class="card">
					<header class="card-header">
						<div class="card-actions">
							Total Category: {{count($categories)}}
						</div>
								...
								@if(count($categories)>0)
									@foreach($categories as $category)
										<tr>
											...
											<td data-title="Date"> {{ $category->created_at  }}</td>
											<td data-title="Title">{{ $category->name }}</td>
											<td data-title="Status" >
												<span class="text-uppercase">{{ $category->status }}</span>
											</td>
											<td data-title="Action">
												<a class="btn btn-sm btn-warning mr-1" data-original-title="Edit" data-toggle="tooltip" title="" href="{{route('category.edit',['id' => $category->id ])}}">
													<i class="fa fa-edit"></i>
												</a>
												<button id="deletebutton" class="btn btn-sm btn-danger mr-1"  data-toggle="modal" data-name="{{$category->name }}" data-id="{{$category->id}}" data-target="#deleteModal">
